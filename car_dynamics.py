...
class Car:
    def __init__(self, world, init_angle, init_x, init_y):
        ...
        self.hull = self.world.CreateDynamicBody(
            ...
            angle = init_angle,
            fixtures = [
                fixtureDef(shape = polygonShape(vertices=[ (x*SIZE*HULL_SCALE,y*SIZE*HULL_SCALE) for x,y in HULL_POLY1 ]), density=1/np.power(HULL_SCALE,2)),
                fixtureDef(shape = polygonShape(vertices=[ (x*SIZE*HULL_SCALE,y*SIZE*HULL_SCALE) for x,y in HULL_POLY2 ]), density=1/np.power(HULL_SCALE,2)),
                fixtureDef(shape = polygonShape(vertices=[ (x*SIZE*HULL_SCALE,y*SIZE*HULL_SCALE) for x,y in HULL_POLY3 ]), density=1/np.power(HULL_SCALE,2)),
                fixtureDef(shape = polygonShape(vertices=[ (x*SIZE*HULL_SCALE,y*SIZE*HULL_SCALE) for x,y in HULL_POLY4 ]), density=1/np.power(HULL_SCALE,2))
