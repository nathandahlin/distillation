...
WINDOW_W = 1200
WINDOW_H = 1000
#original tested parameters WINDOW_W = 600
#original tested parameters WINDOW_H = 400
...
class CarRacingSquare(gym.Env):
    ...
    #self.np_random.seed(0)
            ...
            if alpha <= 0.5*math.pi:
                rad = (a/np.sqrt(2))*(1/(np.sin(alpha)+np.cos(alpha)))
            elif alpha <= math.pi:
                rad = (a/np.sqrt(2))*(1/(np.sin(alpha-0.5*math.pi)+np.cos(alpha-0.5*math.pi)))
            elif alpha <= 1.5*math.pi:
                rad = (a/np.sqrt(2))*(1/(np.sin(alpha-math.pi)+np.cos(alpha-math.pi)))
            elif alpha <= 2*math.pi:
                rad = (a/np.sqrt(2))*(1/(np.sin(alpha-1.5*math.pi)+np.cos(alpha-1.5*math.pi)))
            #rad = rad*TRACK_RAD
            #alphas.append(alpha)
            #rads.append(rad)
            xs.append(np.cos(alpha)*rad)
            ys.append(np.sin(alpha)*rad)
            checkpoints.append( (alpha, rad*math.cos(alpha), rad*math.sin(alpha)) )
        #print(xs)
        #print(ys)

        #with open('square_track.pkl','rb') as track_file:
        #    checkpoints = pkl.load(track_file)
        ...
        #track = track[i1:i2-1]
        track = track[i1:i2]
        # with open('/home/dahlin/Desktop/Car Racing Fixed Track/DQN-master/square_track.pkl','wb') as track_file:
        #     print('saving file')
        #     pkl.dump(checkpoints,track_file)
        ...
        #print('TESTING WELL GLUED TOGTHER')
        ...
        self.track = track
        #print('SUCCESS')
        return True, checkpoints, self.start_alpha
    ...
    def _reset(self):
        ...
        while True:
            success, chceckpoints, start_alpha = self._create_track()
            if success: break
    ...
    def _render(self, mode='human', close=False):
       # print('Render Mode: '+mode)
        ...
        if True:
            ...
            #gl.glViewport(int(WINDOW_W/4), int(WINDOW_H/2.5), int(WINDOW_W/2), int(WINDOW_H/2))
            t.enable()
            self._render_road()
            for geom in self.viewer.onetime_geoms:
                geom.render()
