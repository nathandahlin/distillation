...
class CarRacingCycleRandom(gym.Env):
    metadata = {
    ...
    def __init__(self):
        ...
        checkpoint_files = glob.glob('random_tracks/*.pkl')
        self.num_tracks = len(checkpoint_files)
        ...
        for checkpoint_file in checkpoint_files:
            with open(checkpoint_file,'rb') as f:
                ...
                self.checkpoints.append(chkpts)
                self.start_alphas.append(st_alpha)
    ...
    def _create_track(self,checkpoints,start_alpha):
        # CHECKPOINTS = 24
        ...
        #     rad = (a*b)/np.sqrt((a*np.sin(alpha))**2 + (b*np.cos(alpha))**2)
        #     #if c==0 or c==2:
        #     #    rad = 1.5*TRACK_RAD
        #     #else:
        #     #    rad = 1.5*TRACK_RAD/3
        ...
        #         #print('Start Alpha: '+str(self.start_alpha))
        #         #rad = 1.5*TRACK_RAD
    ...
    def _reset(self):
        ...
        checkpoints = self.checkpoints[self.track_counter]
        start_alpha = self.start_alphas[self.track_counter]
        while True:
            #success, checkpoints = self._create_track()
            success = self._create_track(checkpoints,start_alpha)
            if success: break
            print("retry to generate track (normal if there are not many of this messages)")
        self.car = Car(self.world, *self.track[0][1:4])
        self.track_counter = (self.track_counter+1) % self.num_tracks
