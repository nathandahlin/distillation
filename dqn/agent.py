...
class DQN:
    ...
    def play_episode(self):
        ...
        dones_rollout = []
        eh = (
            self.exp_history if (self.do_training or self.do_rollouts)
        ...
        #plt.figure()
        #plt.imshow(first_frame_pp)

        eh.start_new_episode(first_frame_pp)

        if log_data:
            frames_rollout.append(first_frame_pp)
        ...
        while True:
            ...
            if early_done and self.do_training:
                reward += punishment
            ...
            if log_data:
                actions_rollout.append(action_idx)
                frames_rollout.append(self.process_image(observation))
                rewards_rollout.append(r)
                dones_rollout.append(done)

            eh.add_experience(self.process_image(observation), action_idx, done, reward)
            #frames_out.append(self.process_image(observation))
            ...
            if done:
                if self.do_training or self.do_rollouts:
                    self.episode_counter += 1

                if log_data:
                    #return total_reward, frames_in_episode, np.array(actions_rollout), np.array(frames_rollout[0:len(actions_rollout)]), np.array(rewards_rollout), dones_rollout
                    return total_reward, frames_in_episode, np.array(actions_rollout), np.array(frames_rollout[0:len(actions_rollout)])
                else:
                    return total_reward, frames_in_episode

                #return total_reward, frames_in_episode, frames_out
