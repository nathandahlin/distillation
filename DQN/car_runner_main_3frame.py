...
checkpoint_path = "data/fixed_full_track_3frame"
...
def one_episode():
    ...
    if log_frames:
        pickle.dump(frames_out, open("frames_out"+str(dqn_agent.global_counter)+".p", "wb" ) )
    ...
    return reward,frames, save_cond
...
def main_loop():
    ...
    while True:
        ...
        if dqn_agent.do_training and dqn_agent.episode_counter > train_episodes:
            break
        reward, frames, save_cond = one_episode()
        reward_log.append(reward)
        frame_log.append(frames)
        if save_cond:
            pickle.dump([reward_log,frame_log], open(checkpoint_path+"/frames_rewards"+str(dqn_agent.global_counter)+".p", "wb" ) )
